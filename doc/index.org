   ...
   - if the department match the filter or there's no department filter
    - we have to explode each room (/room<A..F>/) according to the room filter if defined
     - we will skip /empty/ =count= (room column value).
      - into one line which has the format defined [[outputformat][below]].
...
*** Output filename and content <<outputformat>>
...
*** Volumetry    
  ...
  See [[datasets][Creating sample datasets]] to see the size a data samples created.
